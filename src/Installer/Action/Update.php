...
EOD;
        ...
        if (version_compare($moduleVersion, '1.7.5', '<')) {
            ...
            $sql = sprintf("ALTER TABLE %s ADD `type` ENUM('text', 'post', 'article', 'magazine', 'image', 'gallery', 'media', 'download', 'feed') NOT NULL DEFAULT 'text', ADD INDEX (`type`)", $linkTable);
            try {
                $linkAdapter->query($sql, 'execute');
            } catch (\Exception $exception) {
                $this->setResult('db', array(
            ...
            // Alter table : ADD type
            $sql = sprintf("ALTER TABLE %s CHANGE `type` `type` ENUM('text', 'post', 'article', 'magazine', 'image', 'gallery', 'media', 'download', 'feed') NOT NULL DEFAULT 'text', ADD INDEX (`type`)", $storyTable);
            try {
                $storyAdapter->query($sql, 'execute');
            } catch (\Exception $exception) {
                $this->setResult('db', array(
