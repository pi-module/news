...
class ModeratorController extends ActionController
{
    ...
    public function indexAction()
    {
        ...
        // Get topic list row
        $select = $this->getModel('topic')->select()->columns(array('id', 'title', 'slug'))->order(array('id DESC', 'time_create DESC'));
    ...
    public function statusAction()
    {
        ...
        if (in_array($status, array(0, 1))) {
            ...
            if ($row) {
                $message = 'Moderator status time_update successfully';
                $ajaxstatus = 1;
            } else {
                $message = 'Error in time_update moderator status';
                $ajaxstatus = 0;
            }
        } else {
